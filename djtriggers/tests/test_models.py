...
class TriggerTest(TestCase):
    def test_handle_execution_success(self):
        trigger = NormalDummyTriggerFactory()
    ...
    def test_handle_execution_success_use_statsd(self, mock_statsd):
        trigger = NormalDummyTriggerFactory(process_after=timezone.now())
    ...
    def test_handle_execution_failure_dont_raise_when_under_max_retries(self, mock_logger):
        exception = Exception()
        trigger = NormalDummyTriggerFactory(number_of_tries=2)
        original_tries = trigger.number_of_tries
        trigger._handle_execution_failure(exception)
    ...
    def test_handle_execution_failure_tries_exceeded(self, mock_logger):
        exception = Exception()
        trigger = NormalDummyTriggerFactory(number_of_tries=3)
        original_tries = trigger.number_of_tries
        trigger._handle_execution_failure(exception)
    ...
    def test_process_already_processed(self):
        trigger = NormalDummyTriggerFactory(date_processed=timezone.now())
        with raises(AlreadyProcessedError):
            trigger.process()

    def test_process_process_later(self):
        trigger = NormalDummyTriggerFactory(process_after=timezone.now() + timedelta(minutes=1))
